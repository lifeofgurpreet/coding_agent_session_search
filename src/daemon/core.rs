...
pub struct ModelDaemon {
    ...
    /// Warm-search registry: keeps one or more `SearchClient`s resident in
    /// memory keyed by `(data_dir, db_path)`. Populated lazily on first
    /// `Request::Search` (or eagerly at startup when
    /// `CASS_DAEMON_PREWARM_SEARCH=1`). The warm path is the heart of the
    /// "daemon hot search" feature — every CLI search through the daemon
    /// hits this registry instead of re-opening Tantivy + FSVI + HNSW per
    /// query.
...
impl ModelDaemon {
    ...
    pub fn run(&self) -> std::io::Result<()> {
        ...
        // Pre-bind the warm SearchClient (Tantivy reader + FSVI vector index +
        // HNSW graph + filter maps + hydration cache) for this daemon's data
        // directory so the first user query doesn't pay the lazy-bind cost.
        // Gated on `CASS_DAEMON_PREWARM_SEARCH=1` so an embedder/rerank-only
        // daemon (no on-disk search corpus) doesn't try to load nothing.
