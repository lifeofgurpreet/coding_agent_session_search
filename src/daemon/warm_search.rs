...
// Inline POSIX FFI for `mlock(2)` to avoid pulling in the `libc` crate
// just for two calls. See `src/daemon/core.rs::apply_mlockall` for the
// same pattern.
#[cfg(target_os = "linux")]
unsafe extern "C" {
...
#[cfg(target_os = "linux")]
#[inline]
unsafe fn mlock_region(addr: *const u8, len: usize) -> i32 {
    // SAFETY: caller is responsible for `addr..addr+len` covering a valid mapping.
    unsafe { mlock(addr as *const std::ffi::c_void, len) }
}
#[cfg(not(target_os = "linux"))]
#[inline]
unsafe fn mlock_region(_addr: *const u8, _len: usize) -> i32 {
    // No-op on non-Linux platforms; the daemon mlock path is best-effort
    // anyway and Worker 8 is the only deployment that exercises it.
    0
}
...
fn open_warm_client(
    ...
    if let Some(context) = setup.context {
        ...
        let mlock_enabled = std::env::var("CASS_DAEMON_MLOCK_INDEX").as_deref() != Ok("0");
        if mlock_enabled {
            ...
            let mlock_db_enabled = std::env::var("CASS_DAEMON_MLOCK_DB").as_deref() != Ok("0");
            if mlock_db_enabled {
        ...
        let prewarm_ann = std::env::var("CASS_DAEMON_PREWARM_ANN").as_deref() != Ok("0");
        if prewarm_ann {
        ...
        let prewarm_hyd = std::env::var("CASS_DAEMON_PREWARM_HYDRATION").as_deref() != Ok("0");
        if prewarm_hyd {
...
fn mlock_database_file(db_path: &Path) -> Result<u64> {
    ...
    let rc = unsafe { mlock_region(mmap.as_ptr(), len) };
    if rc != 0 {
...
fn mlock_vector_index(data_dir: &Path, embedder_id: &str) -> Result<u64> {
    ...
    for path in &candidates {
        ...
        let file =
            std::fs::File::open(path).with_context(|| format!("mlock: open {}", path.display()))?;
